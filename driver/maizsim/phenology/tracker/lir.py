...
class LeafInductionRate(Tracker):
    def setup(self, pheno, gst_recorder, temperature_recorder, juvenile_leaves):
        self.pheno = pheno
    ...
    def calc(self, T):
        ...
        if self.temperature_recorder._tracker.empty():
            self.temperature_recorder.update(self.gst_recorder.rate)
